...
async function generarPdfFactura(factura) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Peso real de la balanza. Si por alguna razón no viene (facturas muy viejas antes de este
      // campo), se muestra el peso facturado como respaldo para no dejar la columna en blanco.
      const pesoReal = Number(factura.peso_real_lb) > 0 ? Number(factura.peso_real_lb) : Number(factura.peso_facturado_lb || 0);
      doc.font('Helvetica').fontSize(8).fillColor(NAVY)
         .text(`${pesoReal.toFixed(2)} lb`, cX.peso, rowY + 8)
